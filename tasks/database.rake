...
# from git://github.com/mwlang/ramaze-sequel-proto-experimental.git
namespace :db do
  task :load_config do
  ...
  namespace :schema do
    ...
    task :drop => [:load_config, :dump] do
      File.read(File.join(Source, 'db', 'schema.rb'))
      eval(File.read(File.join(Source, 'db', 'schema.rb'))).apply(DB, :down)
    end
    
    desc "loads the schema from db/schema.rb"
    task :load => :load_config do
      eval(File.read(File.join(Source, 'db', 'schema.rb'))).apply(DB, :up)
      latest_version = Sequel::Migrator.latest_migration_version(File.join(Source, 'db', 'migrate'))
    ...
    task :dump => :load_config do
      ...
      schema_file = File.open(File.join(Source, 'db', 'schema.rb'), "w"){|f| f.write(schema)}
    end
  ...
  task :migrate => :load_config do
    Sequel::Migrator.apply(DB, File.join(Source, 'db', 'migrate'))
    File.join(Source, 'db', 'migrate')    
  ...
  namespace :migrate do
    ...
    task :up => :load_config do
      ...
      Sequel::Migrator.apply(DB, File.join(Source, 'db', 'migrate'), version)
      Rake::Task["db:schema:dump"].invoke 
    end
    ...
    task :down => :load_config do
      ...
      Sequel::Migrator.apply(DB, File.join(Source, 'db', 'migrate'), down_version)
      Rake::Task["db:schema:dump"].invoke
    end
  end
